    ...
    for file_path in directory.glob("*.txt"):
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if len(chunks) > 1:
                    logger.info(
                        f"Загружен документ: {file_path.name} ({len(text)} символов) - разбит на {len(chunks)} частей")
...
async def handle_text(message: Message):
    ...
    try:
        ...
        logger.info(
            f"Ответ отправлен пользователю {user_id} (история: {len(conversation_history[user_id]) // 2} сообщений)")
